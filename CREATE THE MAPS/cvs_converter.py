...
# Load the image
image = cv2.imread(r'C:\Users\oscar\oscar\myProjects\polimi\inclusive\InclusivePipeline\CREATE THE MAPS\saloon5.png') # Change this to your desired image path
...
colors_hsv = {
    ...
    "brown1": (np.array([0, 50, 50]), np.array([15, 255, 255])), #brown 1 and 2 are the same seat, and still sometimes not working
    "brown2": (np.array([175, 50, 50]), np.array([180, 255, 255]))
}

#TODO: so for some reason, sometimes who have to ceil and sometimes floor to get good results and it is impossible to know when you whould do when
height=math.ceil(image.shape[0]/17)
width=math.ceil(image.shape[1]/17)

# height=math.floor(image.shape[0]/17)
# width=math.floor(image.shape[1]/17)


# Create masks for each color and find the seats
seat_map = np.zeros((height, width), dtype=int)  # Adjust size based on your image
